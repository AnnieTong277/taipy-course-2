...
def create_network_visualization(top_n):
    ...
    for _, row in df.iterrows():
        ...
        author_ids = [id_.strip() for id_ in author_ids if id_.strip()]
        for author_id in author_ids:
            if author_id in top_authors_df["Author ID"].values:
                for coauthor_id in author_ids:
                    if coauthor_id != author_id and coauthor_id in top_authors_df["Author ID"].values:
                        G.add_edge(author_id, coauthor_id)
    ...
    top_authors = sorted(degree_centrality, key=degree_centrality.get, reverse=True)[:10]
    ...
    for node in nodes:
        ...
        sizes.append(20 if node in top_authors else 10)
    ...
    fig = go.Figure(
        ...
        layout=go.Layout(
            title='<br>Co-Authorship Network',
            titlefont_size=16,
            showlegend=False,
            hovermode='closest',
...
def on_slider_change(state, var_name, var_value):
    ...
    state.author_table = get_top_authors(var_value)[0]  # Only get the DataFrame
    state.network_fig = create_network_visualization(var_value)

def on_button_click(state):
    state.author_table = get_top_authors(state.top_n)[0]  # Only get the DataFrame
...
with tgb.Page() as page:
    with tgb.part(class_name="container"):
        tgb.text("# Author **Analysis**", mode="md")
        ...
        tgb.html("br")
        # Modify the layout to make table and graph equal size
        with tgb.layout(columns="1 1", gap="1rem", class_name="equal-height"):
            with tgb.part(class_name="table-container", style="height: 600px; overflow: auto;"):
                tgb.table(data="{author_table}")
            with tgb.part(class_name="graph-container", style="height: 600px;"):
                tgb.chart(figure="{network_fig}")

# Add some CSS to ensure equal sizing
page.css = """
.equal-height {
    display: flex;
    align-items: stretch;
...
top_n = 8
author_table = get_top_authors(8)[0]
network_fig = create_network_visualization(8)

# Run the application
Gui(page=page).run(title="Author Analysis", dark_mode=False, debug=True, port="auto")
